...
from ABM_07112020_sol1_random_LR_memory_and_CM_changed_and_novelty_bias import songModel
...
modelMods = ['novelty','revolution'] 
memory_conservatisms = [0.1,0.5,0.9]
densities = [0.001,0.01,0.1,1,5,10]
		...
		for memory_conservatism in memory_conservatisms:
			for n_of_immigrant in n_of_immigrants:
				for density in densities:
					expNum = f'ABM3_SimN_{x}_' + f'{modelMod}_' + f'c_{memory_conservatism}_' + f'n_imm_{n_of_immigrant}_' + f'd_{density}_'
...
if __name__ == '__main__':
	pool = mp.Pool(processes=48) # here you can change the number of cores
