...
class songModel(object):
    ...
    def __init__(self,maxSong=50, #Maximum length of a song                    
                    ...
                    probVec=np.array([[0,0.3],[1460,0.7],[3650,0.5],[5840,0.07],[8030,0.3]]), # Singing probabilities based on iteration number/ The pattern follows the seasonal levels of testosterone
        ...
        #Empty array that will later be used to control the likelihood of an agent
        ...
        for numMig in range(0,self.mRuns):
            ...
            for n in range(0,self.i):
                ...
                for j in self.agents_order:
                    ...
                    self.Agents[j].Matrix_Song_Memory(self.Agents,self.intensityMatrix,self.distMat)
                    self.Agents[j].CM_Score()
                    self.Agents[j].weightedGrammar(0,self.Agents,self.modelMode,self.CMscore,self.distMat,self.intensityMatrix) 
...
#            try:
    ...
    def Matrix_Song_Memory(self,Agents,intensityMatrix,distMat):
        ...
        for other in Agents:
            if other.singState == 0:
                ...
                if other.name != self.name and distMat[self.name,other.name] < 50:
        ...
        for a in Agents: 
            ...
            if a.name != self.name and distMat[self.name,a.name] < 50:
                ...
                if (modelMode == 'revolution'):
                    ...
                    scalar = 1 - (intensityMatrix[self.name,a.name]*self.CMscore) #official
                    scalar2 = (intensityMatrix[self.name,a.name]*self.CMscore)
                    self.mat = (self.mat*(scalar) +  G2*scalar2)
